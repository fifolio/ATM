...
interface BullishCoinData {
    historicalBullishDocID: string;
    symbol: string;
    name: string;
...
// This function store today's bullish coins in database and assign them to its historical document
async function POST_bulls(data: BullishCoinData) {
    try {
        const results = await databases.createDocument(
            import.meta.env.VITE_BACKEND_MAIN_DATABASE,
            import.meta.env.VITE_BACKEND_MARKET_BULLS_COLL,
            ID.unique(),
            {
                historicalBullishDocID: data.historicalBullishDocID, // today's historical bullish coins document ID
                symbol: data.symbol,
                name: data.name,
        ...
        console.error("Error storing today's bullish coins:", err);
        return false;
    }
}

export default POST_bulls;