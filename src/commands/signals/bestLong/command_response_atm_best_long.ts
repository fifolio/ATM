...
function command_response_atm_best_long(
    bestLong:
        | { coin: string; symbol: string; market_sentiment: string; reasoning: string; signal_strength: string }
        | { coin: string; symbol: string; market_sentiment: string; reasoning: string; signal_strength: string }[]
) {
    const data = Array.isArray(bestLong) ? bestLong : [bestLong];

    const header = `
╔════════════════════════════════════════════════╗
║ BEST LONG POSITION RECOMMENDATION — ${new Date().toISOString().split('T')[0]} ║
╚════════════════════════════════════════════════╝
`;
...
export default command_response_atm_best_long;
